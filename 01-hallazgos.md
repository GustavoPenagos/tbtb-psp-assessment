...
| H-04 | CA-6 / Notas del documento | **Vacío crítico** | El reporte de adherencia calcula "porcentaje de pacientes contactados dentro del plazo previsto". Sin embargo, las notas finales del documento indican explícitamente: *"La versión definitiva del calendario de seguimiento está pendiente de confirmación con el área médica."* Sin definir el plazo previsto, la métrica de adherencia no se puede calcular correctamente. | El CA-6 es incalculable sin la regla de negocio del calendario. Construir la fórmula con datos de ejemplo llevaría a un reporte incorrecto que se entregaría al laboratorio. | ¿Cuántos días después de la fecha de inicio de tratamiento debe realizarse el primer contacto? ¿Cada cuántos días se programa el siguiente? ¿Hay distintos calendarios por tipo de tratamiento o país? | **CA-6 queda fuera del alcance de entrega.** El modelo de datos contemplará el campo `expected_contact_date` en la entidad de contacto planificado para poder implementar el cálculo en el futuro, pero no se expone el endpoint de reporte de adherencia en esta entrega. |
...
| H-04 | 🔴 Alta | Sí — métrica incalculable | CA-6 |
...
| **CA-6** (Reporte de adherencia) | Depende del calendario de seguimiento que el área médica aún no ha confirmado (H-04). Construir la métrica sin la regla de negocio produce un dato incorrecto. |
